...
void Wwise_RIFF_Vorbis::generate_ogg_header(Bit_oggstream& os, bool * & mode_blockflag, int & mode_bits, unsigned int & mode_count)
{
    ...
    // generate setup packet
    {
        ...
        if (_full_setup)
        {
            // The setup packet is a complete standard Vorbis setup header
            // (only the "vorbis" magic was stripped). Parse it section by
            // section, writing everything through unchanged, so that the
            // mode table (needed for granulepos and mod-packet rebuilds)
            // can be recovered.
            ...
            {
                Bit_uint<16> floor_type;
                ss >> floor_type;
                os << floor_type;
                if (floor_type != 1) throw Parse_error_str("invalid floor type");
            ...
            {
                Bit_uint<16> mapping_type;
                ss >> mapping_type;
                os << mapping_type;
                if (mapping_type != 0) throw Parse_error_str("invalid mapping type");
            ...
            ss >> mode_count_less1;
            unsigned int mode_count_l = mode_count_less1 + 1;
            os << mode_count_less1;

            mode_blockflag = new bool [mode_count_l];
            mode_bits = ilog(mode_count_l-1);
            mode_count = mode_count_l;

            for (unsigned int i = 0; i < mode_count_l; i++)
            {
                ...
                Bit_uint<16> windowtype, transformtype;
                ss >> windowtype >> transformtype;
                os << windowtype << transformtype;
                if (windowtype != 0 || transformtype != 0) throw Parse_error_str("invalid mode window/transform type");
            ...
            ss >> framing;
            os << framing;
            if (framing != 1) throw Parse_error_str("setup framing bit not set");

            // copy any trailing pad bits verbatim
...
void Wwise_RIFF_Vorbis::generate_ogg(ofstream& of)
{
    ...
    {
        generate_ogg_header(os, mode_blockflag, mode_bits, mode_count);
    }
    ...
    // Audio pages
    {
        ...
        // exact granulepos tracking for 2-byte (no stored granule) headers
        ...
        while (offset < _data_offset + _data_size)
        {
            ...
            // exact granulepos for 2-byte packet headers (no stored granule)
            if (_no_granule && mode_blockflag)
            {
                Bit_stream mss(_infile);

                unsigned int mode_number;
                if (_mod_packets)
                {
                    // mod packets: mode number starts at bit 0 of the packet
                    Bit_uintv mode_number_v(mode_bits);
                    mss >> mode_number_v;
                    mode_number = mode_number_v;
                }
                else
                {
                    // standard packets: 1 bit packet type, then mode number
                    Bit_uint<1> packet_type;
                    mss >> packet_type;
                    Bit_uintv mode_number_v(mode_bits);
                    mss >> mode_number_v;
                    mode_number = mode_number_v;
                ...
                {
                    const bool block_flag = mode_blockflag[mode_number];
                    const unsigned int blocksize_pow = block_flag ? _blocksize_1_pow : _blocksize_0_pow;
                    const unsigned int blocksize = 1U << blocksize_pow;

                    if (prev_bs_pow >= 0)
                    {
                        // each subsequent frame advances (B + prev_B) / 4 samples
                        cumulative_samples += (blocksize + (1U << prev_bs_pow)) / 4;
                    }
                    // the first frame is libvorbis priming (0 output samples)
                    prev_bs_pow = blocksize_pow;

                    uint64_t granulepos = cumulative_samples;
                    const bool last_packet = (next_offset >= _data_offset + static_cast<long>(_data_size));
                    if (last_packet && _sample_count > 0 &&
                        _sample_count <= cumulative_samples + blocksize)
                    {
                        granulepos = _sample_count;
                    }
                    os.set_granule(static_cast<uint32_t>(granulepos));
                }

                // fix seek for the first byte handling below
                _infile.seekg(offset);
            }
